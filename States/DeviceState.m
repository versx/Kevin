...
+(void)logout
{
    ...
    if (([[Device sharedInstance] username] == nil || [[[Device sharedInstance] username] isEqualToString:@""]) &&
        ...
        syslog(@"[DEBUG] Preparing min/max level values");
        payload[@"min_level"] = @1;
        payload[@"max_level"] = @29;
        //[payload setValue:[[[Device sharedInstance] minLevel] intValue] forKey:@"min_level"];
        //[payload setValue:[[[Device sharedInstance] maxLevel] intValue] forKey:@"max_level"];
        ...
        [Utils postRequest:backendControllerUrl
            ...
            if (data != nil) {
                ...
                if (firstWarningTimestamp != nil && ![firstWarningTimestamp isEqualToString:@"<null>"]) {
                    syslog(@"[DEBUG] first_warning_timestamp: %@", firstWarningTimestamp);
                ...
                syslog(@"[DEBUG] Checking if username is not empty and ptcToken is not empty.");
                if (username != nil || (ptcToken != nil && ![ptcToken isEqualToString:@""])) {
                    ...
                    if (ptcToken != nil && ![ptcToken isEqualToString:@""]) {
